...
# Compare their run time. You may use the image (fourierspectrum.pgm or university.png) to test 
# the algorithms. Try different gamma values to show the effect.
...
def log_trans(image):
    ...
    transfd_image = np.log1p(image.astype(np.float32))
    runtime = time.time() - start_time
    return transfd_image, runtime

# Power-law transformation
def power_law_trans(image, gamma):
    start_time = time.time()
    transfd_image = np.power(image / 255.0, gamma) * 255.0
    runtime = time.time() - start_time
    return transfd_image, runtime
...
log_transformed, log_runtime = log_trans(image)
...
for i, gamma in enumerate(gamma_values):
    pwr_law_transfd, pwr_law_runtime = power_law_trans(image, gamma)
    axs[0, i+1].imshow(pwr_law_transfd, cmap='gray')
    axs[0, i+1].set_title(f'Power-law (gamma={gamma})')
    axs[0, i+1].text(0.5, -0.2, f'Runtime: {pwr_law_runtime:0.6f}sec', ha='center', transform=axs[0, i+1].transAxes)
    
    # power-law transformation histogram
    axs[1, i+1].hist(pwr_law_transfd.flatten(), bins=256, range=[0,256], color='gray', alpha=0.7)
...
for krnl_size in krnl_sizes:
    kernel = np.ones((krnl_size, krnl_size), dtype=np.float32) / (krnl_size ** 2)
    filtrd_image_avg = cv2.filter2D(img, -1, kernel)
    filtrd_images_avg.append(filtrd_image_avg)

# 5) Remove the noise in the noisy_atrium.png image by using the median filters in spatial domain. 
...
# Median filter 
filtrd_images_median = []

for krnl_size in krnl_sizes:
    filtrd_image_median = cv2.medianBlur(img, krnl_size)
    filtrd_images_median.append(filtrd_image_median)

# Plotting  
plt.figure(figsize=(10,8))

for i, filtrd_image_avg in enumerate(filtrd_images_avg, start=1):
    plt.subplot(len(filtrd_images_avg), 2, i*2-1)
    plt.imshow(filtrd_image_avg, cmap='gray')
    plt.title(f'Average filter\n({krnl_sizes[i-1]}x{krnl_sizes[i-1]})')
    plt.axis('off')

for i, filtrd_image_median in enumerate(filtrd_images_median, start=1):
    plt.subplot(len(filtrd_images_median), 2, i*2)
    plt.imshow(filtrd_image_median, cmap='gray')
    plt.title(f'Median filter \n({krnl_sizes[i-1]}x{krnl_sizes[i-1]})')
    plt.axis('off')

plt.figure(4)
plt.tight_layout()
plt.savefig('noiseRemoval.jpg')
plt.show()

# Machine Vision Project_2 code by Eric L Saldana, March 26 2024
